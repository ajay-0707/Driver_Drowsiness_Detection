...
url = "http://172.70.110.195:8080/shot.jpg"
while True:
	# ret, frame = cap.read()
	...
	# frame = imutils.resize(img, width=400, height=400)
	frame = imutils.resize(img,450,None)
	gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
	height, width = frame.shape[:2]
	# print(height)
	# print(width)
	...
	# else:
	# 	score = score + 1
	# if prediction < 0.5:  # eyes are open
	# 	if fpred >= 0.5:
	# 		score = score + 1
	# 	else:
	# 		score = score - 2
	# else:  # eyes are close
	# 	if fpred >= 0.5:  # eyes closed and yawning
	# 		score = score + 2
	# 	else:
	# 		score = score + 1
	...
	cv2.putText(frame, 'Score:' + str(score), (10, 50), font, 2, (255, 55, 50), 2, cv2.LINE_AA)
	if (score >= 0 and score <= 3):
		cv2.putText(frame, "State: Alert", (10,100), font, 1 ,(0, 255, 0), 1, cv2.LINE_AA)
		...
		cv2.putText(frame, "State: Sleepy", (10,100), font, 1, (0, 255, 155), 1,
		            cv2.LINE_AA)
		cv2.rectangle(frame, (0, 0), (width, height), (0, 0, 255), thicc)
		cv2.imshow('frame', frame)
		mixer.music.stop()
	...
	# 	cv2.putText(frame, "State: Extre", (10, 60), font, 1, (0, 255, 255), 1, cv2.LINE_AA)
	# 	cv2.rectangle(frame, (0, 0), (width, height), (0, 0, 255), thicc)
	# 	cv2.imshow('frame', frame)
	# 	mixer.music.stop()
	# 	if cv2.waitKey(1) & 0xFF == ord('q'):
	# 		break
		...
		if (score > 6):
			cv2.putText(frame, "State: Extremely Sleepy", (10, 100), font, 1, (0, 0, 255), 1, cv2.LINE_AA)
